...
public class ServerRackMenu extends AbstractComputerMenu {
    ...
    public dev.jsc.jscomputronics.module.computing.rack.RackChassis.RackType rackType() {
    ...
    public java.util.List<RackLayout.Unit> mountedUnits() {
        ...
        for (int i = 0; i < RACK_SLOTS; i++) {
            final dev.jsc.jscomputronics.module.computing.rack.RackChassis chassis =
                    dev.jsc.jscomputronics.module.computing.item.ServerItem.chassisOf(serverInBay(i));
            if (chassis != null) {
    ...
    private java.util.List<Integer> claimedSlots(final int topRow, final RackLayout.SlotRole role) {
        final dev.jsc.jscomputronics.module.computing.rack.RackChassis chassis =
                dev.jsc.jscomputronics.module.computing.item.ServerItem.chassisOf(serverInBay(topRow));
    ...
    public dev.jsc.jscomputronics.module.computing.rack.RaidMode raidModeAt(final int topRow) {
        for (final int index : claimedSlots(topRow, RackLayout.SlotRole.GADGET)) {
            final ItemStack stack = frontSlotStack(index);
            if (dev.jsc.jscomputronics.module.computing.item.RackGadgetItem.kindOf(stack)
                    == dev.jsc.jscomputronics.module.computing.item.RackGadgetItem.Kind.RAID_CONTROLLER) {
                return dev.jsc.jscomputronics.module.computing.item.RackGadgetItem.raidMode(stack);
            }
        }
        return dev.jsc.jscomputronics.module.computing.rack.RaidMode.NONE;
    }
    ...
    public String raidLabel(final int topRow) {
        final dev.jsc.jscomputronics.module.computing.rack.RaidMode mode = raidModeAt(topRow);
        if (mode == dev.jsc.jscomputronics.module.computing.rack.RaidMode.NONE) {
            return null;
        }
        int members = 0;
        for (final int index : claimedSlots(topRow, RackLayout.SlotRole.GADGET)) {
