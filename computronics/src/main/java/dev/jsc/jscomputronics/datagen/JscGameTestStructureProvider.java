...
/**
 * Generates the empty structure templates that the GameTests run inside.
 */
public final class JscGameTestStructureProvider implements DataProvider {

    private final PackOutput output;

    public JscGameTestStructureProvider(final PackOutput output) {
        this.output = output;
    }

    @Override
    public String getName() {
        return "J's Computronics GameTest Structures";
    }
    ...
    private CompletableFuture<?> write(final CachedOutput cache, final String name, final CompoundTag tag) {
        final Path path = output.getOutputFolder(PackOutput.Target.DATA_PACK)
                .resolve(JsComputronics.MODID).resolve("structure").resolve(name + ".nbt");
