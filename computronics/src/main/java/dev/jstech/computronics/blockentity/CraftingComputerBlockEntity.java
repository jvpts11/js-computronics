...
public class CraftingComputerBlockEntity extends AbstractComputerBlockEntity
     ...
     * is not a {@link dev.jstech.computronics.block.CraftingComputerBlock} (never happens in
     * practice, but keeps the read total).
     */
    private dev.jstech.core.tier.HardwareEra blockEra() {
        return getBlockState().getBlock()
                instanceof dev.jstech.computronics.block.CraftingComputerBlock cc
                ? cc.era()
    ...
    public java.util.List<dev.jstech.computronics.crafting.NetworkRecipe> machineRecipes() {
        return java.util.Collections.unmodifiableList(machineRecipes);
    }

    public boolean loadMachineRecipe(final dev.jstech.computronics.crafting.NetworkRecipe recipe) {
    ...
    public java.util.List<CraftingSwitchBlockEntity.DeclaredMachine> availableMachines() {
        ...
        while (!queue.isEmpty() && steps++ < 128) {
            ...
            if (level.getBlockState(current).getBlock()
                    instanceof dev.jstech.computronics.block.DataCableBlock cable
    ...
    public java.util.List<dev.jstech.computronics.crafting.CraftingPattern> romPatterns() {
        return java.util.Collections.unmodifiableList(rom);
    }

    public boolean romContains(final dev.jstech.computronics.crafting.CraftingPattern pattern) {
        for (final dev.jstech.computronics.crafting.CraftingPattern existing : rom) {
    ...
    public boolean loadPattern(final dev.jstech.computronics.crafting.CraftingPattern pattern) {
    ...
    protected void saveExtra(final net.minecraft.nbt.CompoundTag tag,
        ...
        if (!rom.isEmpty()) {
            ...
            dev.jstech.computronics.crafting.CraftingPattern.CODEC.listOf()
                    .encodeStart(ops, rom)
        ...
        if (!machineRecipes.isEmpty()) {
            final var ops = net.minecraft.resources.RegistryOps.create(net.minecraft.nbt.NbtOps.INSTANCE, registries);
            dev.jstech.computronics.crafting.NetworkRecipe.CODEC.listOf()
