...
public sealed interface Binding {
    ...
    record Variable(String name, TypeSymbol type, boolean parameter) implements Binding {
    }

    /** A field, a property, an event or a method of a type, with its arguments already filled in. */
    record Member(MemberSymbol member, TypeSymbol type) implements Binding {
    }

    /** A type used where a receiver goes, as in a call of one of its static members. */
    record TypeName(TypeSymbol type) implements Binding {
    }
}
