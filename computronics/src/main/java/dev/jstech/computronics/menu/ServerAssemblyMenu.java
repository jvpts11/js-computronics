...
public class ServerAssemblyMenu extends AbstractComputerMenu {
    ...
    private dev.jstech.computronics.rack.RackChassis chassis() {
        final var chassis = ServerItem.chassisOf(owner.getItemInHand(hand));
        return chassis != null ? chassis : dev.jstech.computronics.rack.RackChassis.SERVER;
    }
    ...
    public dev.jstech.computronics.hardware.ComputerBuild currentBuild() {
    ...
    public dev.jstech.core.tier.HardwareEra hardwareEra() {
        final dev.jstech.computronics.hardware.ComputerBuild build = currentBuild();
    ...
    public void setServerName(final String name) {
        ...
        ServerItem.setCustomName(owner.getItemInHand(hand),
                capped.length() > dev.jstech.computronics.operation.payload.RenameServerPayload.MAX_LEN
                        ? capped.substring(0,
                            dev.jstech.computronics.operation.payload.RenameServerPayload.MAX_LEN)
                        : capped);
        broadcastChanges();
    }
