 ...
 * one thing that cannot — a factory that builds the {@link DesktopApp} on the client. Together they are
...
public final class ProgramClient {
    ...
    @FunctionalInterface
    public interface DesktopAppFactory {
        DesktopApp create(BlockPos host, BlockPos monitorPos, ResourceLocation osId);
    }

    private static final Map<ResourceLocation, DesktopAppFactory> FACTORIES = new LinkedHashMap<>();
    ...
    public static void register(final ResourceLocation id, final DesktopAppFactory factory) {
        FACTORIES.put(id, factory);
    }

    /** The window factory for {@code id}, or {@code null} when the program opens no desktop window. */
    public static DesktopAppFactory factory(final ResourceLocation id) {
