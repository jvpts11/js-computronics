...
import dev.jstech.computronics.gui.layout.NetworkInteractorLayout;
...
import dev.jstech.computronics.operation.payload.NetworkInteractorPayload;
import dev.jstech.computronics.operation.payload.NetworkItemEntry;
import dev.jstech.computronics.operation.payload.NiDepositPayload;
import dev.jstech.computronics.operation.payload.NiGridClickPayload;
import dev.jstech.computronics.operation.payload.NiSelectPayload;
import dev.jstech.computronics.operation.payload.RequestNetworkInteractorPayload;
import dev.jstech.computronics.operation.payload.RequestNiServersPayload;
...
public final class NetworkInteractorApp implements InventoryBandApp {
    ...
    private final List<dev.jstech.computronics.operation.payload.NetworkServersPayload.ServerEntry>
    ...
    private dev.jstech.computronics.operation.payload.CraftCatalogPayload.Entry craftPopup;
    ...
    private dev.jstech.computronics.operation.payload.CraftPlanPayload craftPlan;
    ...
    public static void acceptCraftPlan(
            final dev.jstech.computronics.operation.payload.CraftPlanPayload plan) {
    ...
    public static void acceptServers(final List<dev.jstech.computronics.operation
    ...
    private final List<dev.jstech.computronics.operation.payload.OperationRecord>
    ...
    public static void acceptOps(final List<dev.jstech.computronics.operation
    ...
    public static void acceptActiveOps(final List<dev.jstech.computronics.operation
    ...
    private void renderDetails(final GuiGraphics g, final Font font, final int x, final int y,
        ...
        final dev.jstech.computronics.storage.StorageKey key = e.key();
    ...
    private List<dev.jstech.computronics.operation.payload.OperationRecord> allOps() {
        final List<dev.jstech.computronics.operation.payload.OperationRecord> all =
    ...
    private static String opTypeLabel(final byte type) {
        return dev.jstech.computronics.program.OperationPalette.labelFor(type);
    }
    ...
    private static int opTypeColor(final byte type) {
        ...
        // everywhere.
        return dev.jstech.computronics.program.OperationPalette.colorFor(type);
    }
     ...
     */
    public java.util.Optional<dev.jstech.computronics.storage.StorageKey> cursorDepositEntry(
    ...
    public int shiftInsertTarget() {
        if (tab == TAB_NETWORK) {
            return dev.jstech.computronics.operation.payload
    ...
    private void requestCraftPlan() {
        if (craftPopup != null) {
            PacketDistributor.sendToServer(
                    new dev.jstech.computronics.operation.payload.CraftPlanRequestPayload(
    ...
    private void submitCraft(final boolean partial) {
        if (craftPopup != null) {
            ...
            PacketDistributor.sendToServer(
                    new dev.jstech.computronics.operation.payload.CraftSubmitPayload(
    ...
    private static String amount(final dev.jstech.computronics.storage.StorageKey key, final long n) {
        return key.isItem() ? formatCount(n) : formatCount(n) + " mB";
    }

    /** The exact amount with its unit, for a tooltip. */
    private static String amountLabel(final dev.jstech.computronics.storage.StorageKey key, final long n) {
        return key.isItem()
    ...
    private static net.minecraft.resources.ResourceLocation dataId(
            final dev.jstech.computronics.storage.StorageKey key) {
        if (key.isFluid()) {
    ...
    private static String dataLabel(final long weight) {
        final long mb = weight * 4L / dev.jstech.computronics.storage.StorageKey.MB_EQ_PER_ITEM;
        if (mb < 1024L) {
            return mb + " MB";
        }
        if (mb < 1024L * 1024L) {
