...
public final class DiskFilesystem {
    ...
    static boolean installerLocked(final ItemStack volume) {
        if (!(volume.getItem() instanceof dev.jstech.computronics.os.media.MediaItem)) {
            return false;
        }
        final dev.jstech.computronics.os.media.MediaKind kind =
                dev.jstech.computronics.os.media.MediaItem.kind(volume);
        return kind != dev.jstech.computronics.os.media.MediaKind.DATA
                && dev.jstech.computronics.os.media.MediaItem.payload(volume) != null;
    }
