 ...
 */
package dev.jstech.computronics.os.media;
...
public class FormattedMediaItem extends MediaItem {
    ...
    public void appendHoverText(final ItemStack stack, final TooltipContext context,
        ...
        if (!fs.files().isEmpty()) {
            dev.jstech.computronics.os.fs.FilesystemTooltip.append(fs, tooltip);
        ...
        switch (MediaItem.kind(stack)) {
            case OS_INSTALL -> {
                final dev.jstech.computronics.os.OsDef os =
                        payload == null ? null : dev.jstech.computronics.os.OsRegistry.getOs(payload);
                if (payload != null) {
                    ...
                    tooltip.add(Component.literal("Bootable installer" + (os == null ? "" : " · "
                            + dev.jstech.computronics.os.MinSpecTooltip.eraLabel(os.minEra()) + " era"))
                            .withStyle(ChatFormatting.GREEN));
                    tooltip.addAll(dev.jstech.computronics.os.MinSpecTooltip.osMinSpec(payload));
                ...
                if (payload != null) {
                    ...
                    tooltip.add(Component.translatable("program.jsc." + payload.getPath())
                            ...
                            .append(spec == null ? Component.empty() : Component.literal("  "
                                    + spec.houseOr(dev.jstech.computronics.os.SoftwareHouse.MIDSOFT).name()
                                    + " · " + dev.jstech.computronics.os.Branding.year(spec.era()))
                                    .withStyle(ChatFormatting.GRAY)));
                    ...
                    tooltip.add(Component.literal(insertHint(format) + ": run "
                            + (spec != null && spec.platforms().equals(java.util.Set.of(
                                    dev.jstech.computronics.os.Platform.LINUX))
     ...
     */
    public static List<String> installCommands(final dev.jstech.computronics.os.ProgramSpec spec) {
        final List<String> commands = new java.util.ArrayList<>(2);
        if (spec.platforms().contains(dev.jstech.computronics.os.Platform.FRAMES)) {
            commands.add(command(dev.jstech.computronics.os.PackageManagerKind.PCKMGR, spec));
        }
        if (spec.platforms().contains(dev.jstech.computronics.os.Platform.LINUX)) {
            commands.add(command(dev.jstech.computronics.os.PackageManagerKind.APT, spec));
        }
        return commands;
    }

    private static String command(final dev.jstech.computronics.os.PackageManagerKind manager,
                                  final dev.jstech.computronics.os.ProgramSpec spec) {
