 ...
 *
 * <p>Implementing this instead of overriding {@link DataNetworkConnectable#connectsOnFace} per block keeps the rear-only rule in one place, so the cable's rendered connection and the device's network attachment can never drift apart. {@link DataNetworkConnectable} itself defaults to accepting every face (the Mainframe's behaviour); this narrows that to the rear.
 */
public interface RearFacingDataPort extends DataNetworkConnectable {
