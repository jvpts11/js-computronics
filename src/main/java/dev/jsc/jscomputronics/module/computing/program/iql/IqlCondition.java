 ...
 */
package dev.jsc.jscomputronics.module.computing.program.iql;
 ...
 */
public sealed interface IqlCondition
        permits IqlCondition.Comparison, IqlCondition.And, IqlCondition.Or, IqlCondition.Not {
     ...
     */
    static String firstValue(final IqlCondition tree, final String field) {
        return switch (tree) {
            case null -> null;
     ...
     */
    static String itemNameFilter(final IqlCondition tree) {
    ...
    record Comparison(String field, Op op, String value) implements IqlCondition {
        @Override
    ...
    record And(IqlCondition left, IqlCondition right) implements IqlCondition {
        @Override
        public boolean matches(final Function<String, String> row) {
    ...
    /** Either side holds. */
    record Or(IqlCondition left, IqlCondition right) implements IqlCondition {
        @Override
        public boolean matches(final Function<String, String> row) {
            return left.matches(row) || right.matches(row);
        }
    ...
    record Not(IqlCondition inner) implements IqlCondition {
        @Override
        public boolean matches(final Function<String, String> row) {
