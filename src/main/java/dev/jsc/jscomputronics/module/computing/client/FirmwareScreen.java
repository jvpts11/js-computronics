...
import dev.jsc.jscomputronics.module.computing.operation.payload.FirmwareActionPayload;
...
import dev.jsc.jscomputronics.module.computing.operation.payload.RequestFirmwareStatePayload;
...
public class FirmwareScreen extends Screen {
        ...
        if (page == PAGE_STORAGE) {
            if (state != null && state.raid().present()
                    && (storageSel == 0 || state.raid().drives()
                            >= dev.jsc.jscomputronics.module.computing.rack.RaidMode
    ...
    private void renderCliBios(final GuiGraphics g, final int x, final int y, final int mouseX, final int mouseY) {
        ...
        final int tx = x + 14;
        int ty = y + 10;
        final String title = dev.jsc.jscomputronics.module.computing.os.Branding.biosBanner(era());
    ...
    public boolean keyPressed(final int key, final int scan, final int mods) {
        switch (key) {
            ...
            case 264 -> { // Down
                if (page == PAGE_STORAGE) {
                    storageSel = Math.min(
                            dev.jsc.jscomputronics.module.computing.rack.RaidMode.values().length - 1,
                            storageSel + 1);
                } else {
