...
public final class MekanismHardwareMatrixGameTests {
    ...
    public static void singleQueue_secondCraftOnTheSameMachineWaitsForTheQueue(final GameTestHelper helper) {
        // One CPU, no GPU: one queue. Two frame crafts each start a machine step; only one machine step can
        // hold the queue, so the second craft stays PENDING until it frees up — visible in the operation log.
        ...
        helper.startSequence()
                .thenExecuteAfter(SETTLE + 2, () -> {
                    ...
                    // The Machines tab allows two jobs on the infuser, so the queue is the only limit.
                    rig.net().cc().setMachineConfig(INFUSER.toString(), new CraftingComputerBlockEntity.MachineConfig(2, false, false));
                })
                .thenExecuteAfter(SETTLE + 2, () -> {
                ...
                .thenExecuteAfter(60, () -> {
                    ...
                    helper.assertTrue(count(records, infused, OperationRecord.STATUS_PROCESSING) == 1,
                            "exactly one machine step may run on one queue; active=" + records);
                    helper.assertTrue(count(records, infused, OperationRecord.STATUS_PENDING) == 1,
                            "the second machine step must wait PENDING for the queue; active=" + records);
                    // Both crafts got past the computer claim: the first let it go while its machine works.
                    helper.assertTrue(count(records, frame, OperationRecord.STATUS_PROCESSING) == 2,
                            "both crafts must be under way, neither blocked on the one computer; active=" + records);
                })
                .thenSucceed();
    }

    @GameTest(template = ARENA, timeoutTicks = 600)
    public static void gpu_secondCraftOnTheSameMachineRunsOnItsOwnQueue(final GameTestHelper helper) {
        // The same two crafts on a Mainframe with a GPU (two queues): both machine steps run side by side.
        final MekanismRig.Rig rig = MekanismRig.build(helper, INFUSER);
        final StorageKey frame = MekanismRig.itemKey(FRAME);
        final StorageKey infused = MekanismRig.itemKey(ALLOY_INFUSED);
        helper.startSequence()
                ...
                .thenExecuteAfter(60, () -> {
                    ...
                    helper.assertTrue(count(records, infused, OperationRecord.STATUS_PROCESSING) == 2,
                            "both machine steps must run, one per queue; active=" + records);
    ...
    public static void mainframeOff_refusesTheRequest(final GameTestHelper helper) {
        ...
        helper.startSequence()
                ...
                .thenExecuteAfter(SETTLE, () -> {
                    ...
                    final var response = dev.jsc.jscomputronics.module.computing.program.cli.CliCommands.newShell(50)
